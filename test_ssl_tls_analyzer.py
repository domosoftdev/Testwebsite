...
class TestSSLAnalyzer(unittest.TestCase):
    ...
    def test_analyze_host_success(self, MockScanner):
        ...
        # We only need to mock the final attributes that are accessed in the code
        ...
        leaf_cert.subject = [MagicMock(oid=MagicMock(_name='common_name'), value='test.com')]
        leaf_cert.issuer = [MagicMock(oid=MagicMock(_name='common_name'), value='Test CA')]
        ...
        mock_scan_result.scan_result.ssl_3_0_cipher_suites.result = None
        mock_scan_result.scan_result.tls_1_0_cipher_suites.result = None
        mock_scan_result.scan_result.tls_1_1_cipher_suites.result = None
        ...
        # get_results returns a generator, so our mock should return an iterator
        ...
        self.assertEqual(results['certificate_details']['subject']['common_name'], 'test.com')
        self.assertFalse(results['certificate_details']['is_expired'])
        self.assertTrue(results['protocol_analysis']['protocols']['TLSv1.2'])
        self.assertTrue(results['protocol_analysis']['protocols']['TLSv1.3'])
        self.assertEqual(len(results['protocol_analysis']['weak_protocols_found']), 0)
